...
def parse_color(string, start=0, second=False):
    ...
    if color:
        ...
        if color.end != length:
            ...
            m = tools.RE_DIFF_METHOD.match(string, start)
            if m:
                method = m.group(1)
                start = m.end(0)
...
class ColorDifferenceInputHandler(tools._ColorInputHandler):
    """Handle color inputs."""
...
class ColorHelperDifferenceCommand(_ColorMixin, sublime_plugin.TextCommand):
    ...
    def run(
        self, edit, color_difference, initial=None, on_done=None, **kwargs
    ):
        """Run command."""

        colors, _ = evaluate(color_difference)
        color = None
        if colors:
            color = colors[0]
    ...
    def input(self, kwargs):  # noqa: A003
        ...
        return ColorDifferenceInputHandler(self.view, **kwargs)
